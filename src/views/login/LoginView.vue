      ...
      <v-col cols="11" style="height: 70vh">
        <v-card height="100%" color="secondary" class="text-center px-8 py-2">
          ...
          <v-row style="height: 15%">
            <v-col cols="12">
              <v-text-field label="Email" v-model="auth.email" variant="outlined" dense></v-text-field>
            </v-col>
          </v-row>
          <v-row align="center" style="height: 13%">
            <v-col cols="12">
              <v-text-field label="Senha" v-model="auth.senha" type="password" variant="outlined"></v-text-field>
            </v-col>
          </v-row>
          <v-row align="center" style="height: 30%">
            <v-col cols="12">
              <v-btn width="100%" large color="primary" class="mb-2" block:loading="loadingAuth"
                  @click="Authenticate()">entrar</v-btn>
...
<script setup lang="ts">
  ...
  async function Authenticate() {
    try {
      ...
      const response = await UserServices.login(auth.value);
